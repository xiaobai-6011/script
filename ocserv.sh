...
5. 禁止利用本程序从事非法网络活动
6. 禁止使用本程序进行商业盈利行为
7. 使用本程序产生的任何后果由使用者自行承担
8. 作者不对任何使用行为承担法律责任
9. 作者保留随时修改本协议的权利
10. 如不同意上述条款，请立即停止使用
【免责声明】
1. 本程序按现状提供，不提供任何明示或暗示的保证
2. 作者不对程序的安全性、稳定性、完整性做任何承诺
3. 使用本程序的风险由使用者自行承担
4. 作者不对因使用本程序造成的任何损失负责
5. 本程序可能存在未知的漏洞和风险
6. 使用者应自行负责确保符合当地法律法规
7. 作者有权在任何时候终止对本程序的支持"
EOF
    ...
    if [[ ! -f server-cert.pem ]]; then
        # 获取外网IP作为CN
        SERVER_IP=$(curl -s -4 ifconfig.me || curl -s -4 icanhazip.com || echo "vpn.example.com")
        RANDOM_ORG=$(gen_random 5)
        openssl req -newkey rsa:2048 -nodes -keyout server-key.pem -x509 -days 365 -out server-cert.pem -subj "/CN=${SERVER_IP}/O=${RANDOM_ORG}" 2>/dev/null
        chmod 600 server-key.pem
        echo -e "\033[32m[√]\033[0m 证书已生成: CN=${SERVER_IP}, O=${RANDOM_ORG}"
    fi
